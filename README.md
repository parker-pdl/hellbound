...
A 5-minute hell-corridor runner with a demon on your heels by Parker Data Link LLC.
...
- `demon.glb` — the chasing demon (same model as Hellscape Horror level one)
- `three.min.js`, `GLTFLoader.js` — local copies (CDN used as fallback)

## Level rules
- Survive **5:00** on the clock to win ("YOU MADE IT OUT").
...
- The demon chases you; every Lament Box throws him back.
...
## Deploy
Static files — upload all files together (Cloudflare Pages or the Worker's assets).
